...
export default function CsvAudienceSection({ campaignId }) {
  ...
  // Build the mapping dictionary the backend expects (our conventions):
  //  - Body variables -> "parameterN"
  //  - Header text    -> "headerparaN"
  //  - Button URL dyn -> "buttonparaN"
  // Values are CSV column names, or "constant:<value>" for constants.
  const buildMappingDict = () => {
    const dict = {};

    // Body placeholders -> parameterN
    ...
    // Header & buttons (explicit keys returned by schema)
    for (const [k, v] of Object.entries(keyToColumn || {})) {
      if (!v) continue;
      // Keep only non-body keys here; body handled above
