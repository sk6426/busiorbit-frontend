...
//   // 🆕 Sender selection (from WhatsAppPhoneNumbers)
//   const [senders, setSenders] = useState([]); // [{id, provider, phoneNumberId, displayNumber}]
//   const [selectedSenderId, setSelectedSenderId] = useState("");
...
//     (async () => {
//       try {
//         // Expected API: GET /api/whatsapp/senders/{businessId}
//         // const r = await axiosClient.get(`/whatsapp/senders/${businessId}`);
//         const r = await axiosClient.get(
//           `/WhatsAppSettings/senders/${businessId}`
//         );
//         const raw = Array.isArray(r.data) ? r.data : r.data?.items || [];
...
//           const phoneNumberId = x.phoneNumberId;
//           const displayNumber =
//             x.displayNumber || x.phoneNumber || x.phoneNumberId;
//           const id = x.id ?? `${provider}|${phoneNumberId}`;
//           return { id, provider, phoneNumberId, displayNumber };
//         });
//         setSenders(normalized);
//         if (normalized.length === 1) setSelectedSenderId(normalized[0].id);
//       } catch {
...
//       // 🆕 Sender (provider auto-derived from chosen number)
//       provider: String(selectedSender.provider || "").toUpperCase(), // "PINNACLE" | "META_CLOUD"
//       phoneNumberId: selectedSender.phoneNumberId,
//     };
...
//                 {senders.map(s => (
//                   <option key={s.id} value={s.id}>
//                     {s.displayNumber} • {s.provider}
//                   </option>
//                 ))}
//               </select>
//               <p className="mt-1 text-[11px] text-gray-400">
//                 The provider is chosen automatically based on the selected
//                 number.
//               </p>
//             </div>
